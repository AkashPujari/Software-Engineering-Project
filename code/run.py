...
import logging,os,re,json,random,argparse,glob,shutil
import numpy as np
...
def get_example(item):
    ...
    if url1 in cache:
        ...
        try:
            ...
            code=""
        code1=tokenizer.tokenize(code) #tokenizing the code
    if url2 in cache:
        ...
        try:
            ...
            code=""
        code2=tokenizer.tokenize(code) #tokenizing the code
        
    return get_features(code1,code2,label,url1,url2,tokenizer,args,cache)
...
def get_features(code1_tokens,code2_tokens,label,url1,url2,tokenizer,args,cache):
    ...
    tokens_src=code1_tokens+code2_tokens
    ids_src=code1_ids+code2_ids
    return Features(tokens_src,ids_src,label,url1,url2)
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=512,pool=None):
        ...
        url_to_code={}
        with open('/'.join(index_filename.split('/')[:-1])+'/data.jsonl') as input_:
            for x in input_:
                x=x.strip()
                js=json.loads(x)
        ...
        data=[]
        cache={}
        input_=open(index_filename)
        with open(index_filename) as input_:
            for x in input_:
                x=x.strip()
                url1,url2,label=x.split('\t')
...
def train(args, train_dataset, model, tokenizer,pool):
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon) #AdamW optimizer
    ...
    if args.local_rank != -1: #this one works
...
def test(args, model, tokenizer, prefix="",pool=None,best_threshold=0):
    ...
    logits=np.concatenate(logits,0)
    y_preds=logits[:,1]>best_threshold
    cur_path = os.path.join(args.output_dir,"predictions.txt")
